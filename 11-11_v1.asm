...
.macro SelecionaCelula		# Recebe novos valores de $s1 e $s0, (pra onde irá "andar" )
	...
	mul $t2, $s3, 10240	#
	SelecionaCelula_aux ($s6, $t1, $t2)	# apaga seleção anterior
	mul $t1, $s0, 136			# setando parametros da celula à selecionar
	nop
	mul $t2, $s1, 10240			#
	nop
	SelecionaCelula_aux ($s7, $t1, $t2)	# selecionando celula
	nop
.end_macro
#####################################################################################
###############################################################################################################
.macro SelecionaCelula_aux (%sx, %x1, %x2)  #cor, cord X, comprimento da linha
		add $a1, $zero, 136		# Calculate ending possition
		add $a2, $a1, $gp		# Add global pointer
		
		add $a1, $zero, $gp		# Set loop var to global pointer
		add $t0, $zero, 0		# soma 1024 a t0, pula linha abaixo
		add $t0, $t0, 14400
		add $t0, $t0, %x1
		add $t0, $t0, %x2
		add $a2, $a2, $t0
		add $a1, $a1, $t0		#
		add $a3, $a1, 10240		# a3 pinta a linha de baixo tracejada, 8192 Ã© o espaÃ§o entre as linhas
...
DHLT_Loop:
		...
		add $a0, $a1, 136		# a0 desenha a linha tracejada oposta a que $a1 desenha com espaÃ§o de uma celula
		add $a2, $a1, 9216
DVTL_Loop:
